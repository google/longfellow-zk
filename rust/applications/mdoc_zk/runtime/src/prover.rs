...
use mdoc_zk_circuits::cbor::mdoc::{try_parse_mdoc, MsoLayoutError, ParsedMdoc};
use runtime_algebra::{
...
pub fn run_mdoc_prover_inner<RNG: RandomEngine>(
    ...
    let parsed: ParsedMdoc<runtime_algebra::RuntimeNat<4>> =
        try_parse_mdoc(mdoc_bytes, transcript, doc_type).map_err(|err| match err {
            MsoLayoutError::UnsupportedEncoding => MdocProverErrorCode::MsoDecodingFailure,
            MsoLayoutError::TooBig => MdocProverErrorCode::TaggedMsoTooBig,
        })?;
